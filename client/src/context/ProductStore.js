...
const ProductStore = create((set, get) => ({
  ...
  addNewProduct: async (isValidate, validatePayload) => {
    ...
    try {
      if (isValidate) {
        const newProduct = await axiosInstance.post(addProduct, validatePayload);
  ...
  fetchProductSingle: async (id) => {
    try {
      set({ addLoading: true })
      const product = await axiosInstance.get(fetchSingleProduct.replace(':id', id))
  ...
  updateProduct: async () => {
    const { selectedItem, setUpdatedItem, setEditProductModal, setChangesModal, isScreenLoading } =
      ModalStore.getState()
    try {
      const { productName, quantity, category, price, image, productBranch } = get()
      isScreenLoading(true)
      const payload = {
