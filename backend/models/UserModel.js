...
userSchema.statics.fetchLockedAccounts = async function (funcCd, selectedBranch) {
  const settings = await SystemSettings.getSettings()
  if (funcCd === FETCHLOCKEDACCOUNTNOBRANCH && !selectedBranch) {
    return await this.find({
      loginAttempts: { $gte: settings.generalSettings.maxLoginAttempts }
    }).select('-password')
  }
  return await this.find({
...
userSchema.statics.unlockAccounts = async function (ids) {
  const idList = ids.map(user => user.id);
