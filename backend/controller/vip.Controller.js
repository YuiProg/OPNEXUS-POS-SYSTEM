...
export const addVip = async (req, res) => {
    try {
        ...
        if (data._id && data._id.length > 5) {
            return ApiResponseModel(res, ERROR, "VIP ID must not exceed 5 characters");
        }

        if (data._id) {
            const existingVip = await Vip.getVipById(data._id);
            if (existingVip && existingVip.length > 0) { 
                return ApiResponseModel(res, ERROR, "VIP with this ID already exists");
            }
        }
